...
plt.title('Формула 1 - 2022 Сезон\nКоличество побед в квалификации', fontsize=19, fontweight='bold', color='#bbbbbb')
for i in range(len(polePositions)):
    plt.text(polePositions[i]-0.4, i+0.15, polePositions[i], fontsize=18, fontweight='bold', color='k')
plt.xlabel('ПОБЕДА В КВАЛИФИКАЦИИ', fontsize=14, fontweight='bold', color='#bbbbbb')
plt.ylabel('ГОНЩИКИ', fontsize=14, fontweight='bold', color='#bbbbbb')
plt.xticks(color='#bbbbbb')
plt.yticks(color='#bbbbbb')
plt.axvline(0, color='#bbbbbb')

plt.savefig('data_plot/plot_5.png')
